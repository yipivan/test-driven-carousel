...
describe('HasIndex()', () => {
  ...
  // START_HIGHLIGHT
  it('has initial `index` state equal to the `defaultIndex` prop', () => { //<callout id="co.default-index" />
    expect(wrapper.state('index')).toBe(0);
    const wrapper2 = shallow(<MockComponentWithIndex defaultIndex={1} />);
    expect(wrapper2.state('index')).toBe(1);
  });

  it('always has `index` state equal to the `index` prop', () => { //<callout id="co.prop-controls-state" />
  ...
  it('allows `index` state to change if the `index` prop is unset', () => { //<callout id="co.unset-index" />
    const wrapperWithInitialIndex = shallow(
      <MockComponentWithIndex index={1} />
    );
  ...
  it('calls `onIndexChange` on decrement/increment', () => { //<callout id="co.change-handler" />
